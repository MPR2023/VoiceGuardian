...
const AudioUploader: React.FC = () => {
  ...
  const unsupportedExtensions = ['.m4a', '.aac', '.flac', '.webm'];
  ...
  const isFileTypeUnsupported = (file: File): boolean => {
    const fileName = file.name.toLowerCase();
    const fileExtension = '.' + fileName.split('.').pop();
  ...
  const processAudioFile = useCallback(async (file: File) => {
    ...
    if (isFileTypeUnsupported(file)) {
      setFileTypeError('This audio file format is not currently supported (but coming soon). Please upload an MP3, WAV, or OGG file.');
      return;
    }
    ...
    try {
      ...
      const audioFileData = {
        ...
        name: file.name,
        duration,
        blob: file, // Use original file directly
        size: file.size,
        uploadedAt: new Date()
      };
      ...
      alert(`Audio processing failed: ${error instanceof Error ? error.message : 'Unknown error'}. Please try a different file.`);
    }
  }, [addAudioFile]);
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      {/* Error Display */}
      {(recordingError || fileTypeError) && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 flex items-center space-x-3">
          <AlertCircle className="h-5 w-5 text-red-600 flex-shrink-0" />
          <div>
            <p className="text-red-800 font-medium">
              {fileTypeError ? 'Unsupported File Format' : 'Recording Error'}
            </p>
            <p className="text-red-700 text-sm">{fileTypeError || recordingError}</p>
          </div>
        </div>
      )}
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-1 text-sm md:text-base">
                {isDragReject 
                  ? 'Please upload audio files only (MP3, WAV, OGG)'
                  : 'or click to browse your files'
                }
              </p>
              <p className="text-sm text-gray-500 mt-2">
                Supports MP3, WAV, and OGG files only. (M4A, AAC, FLAC, and WebM are currently NOT supported.) Max 100MB.
              </p>
            </div>
            ...
            <div className="flex items-center space-x-3 min-w-0 flex-1">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-center space-x-2 text-sm text-gray-600">
                  ...
                  <span>•</span>
                  <span>{uploadedFile.type || 'Unknown format'}</span>
                </div>
              </div>
            </div>
            <button
